...
test('home search uses the page search bar', async ({ worker }) => {
	worker.use(
...
mobileTest('home search opens an empty mobile overlay', async () => {
	const visualPage = await renderVisualPage({
		path: '/',
		component: routeComponent(HomeRoute),
		waitFor: (screen) => screen.getByRole('heading', { name: /imdbgraph/i }),
	...
	await expect
		...
		.toBeVisible()
	await visualPage.expectScreenshot('home-search-overlay-empty')
})

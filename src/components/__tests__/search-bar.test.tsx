import { mobileTest, test } from '@config/test/browser'
...
describe('searchbar tests', () => {
	...
	test('resets selection when refined results keep the same first show', async ({
		worker,
	}) => {
		worker.use(
		...
		const screen = await render(<SearchBar />, { wrapper: MockRouter })
		await userEvent.fill(screen.getByRole('combobox'), 'avat')
	...
	test('renders a blurred backdrop behind the open search with the navbar variant', async () => {
		const screen = await render(<SearchBar variant="navbar" />, {
			wrapper: MockRouter,
		})
	...
	test('clicking outside closes the dropdown and keeps the query', async () => {
		// On mobile the standalone search opens the overlay, which dismisses
	...
	test('caps the suggestion list at five results', async ({ worker }) => {
		worker.use(
		...
		expect(screen.getByText(shows[5]!.title).elements()).toHaveLength(0)
	})
...
describe('searchbar screenshots', () => {
	async function renderSearchBar(
		...
		await resizeScreenshotViewport(width, height)
